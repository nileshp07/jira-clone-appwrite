...
const app = new Hono()
	...
	.post('/', zValidator('form', createWorkspaceSchema), sessionMiddleware, async (c) => {
		...
		if (image instanceof Blob) {
			...
			const uploadedFile = new File([image], `${ID.unique()}.${extStr.indexOf('svg') !== -1 ? 'svg' : extStr}`, {
				type: image.type,
			});

			const file = await storage.createFile(IMAGES_BUCKET_ID, ID.unique(), uploadedFile);
